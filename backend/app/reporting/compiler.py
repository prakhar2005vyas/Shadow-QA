...
from ..models import Run, Finding, Report
...
async def compile_findings(run_id: int, db_session: Session) -> None:
    ...
    """
    run = db_session.exec(select(Run).where(Run.id == run_id)).first()
    if not run:
        logger.error("Run %d not found, aborting report compilation", run_id)
        return
    ...
    for finding in findings:
        ...
        if finding.is_mock:
            raw_text = "> ⚠️ **MOCK RUN** — findings generated by scripted mock_vlm.py, not real page analysis.\n\n" + raw_text
